...
// Original content from utils.ts related to Farcaster metadata
...
export async function getFarcasterMetadata(): Promise<FrameMetadata> {
  ...
  if (secretEnvVars) {
    ...
    const encodedSignature = Buffer.from(signature.toString(), "utf-8").toString("base64url"); // ensure signature is string before Buffer.from
