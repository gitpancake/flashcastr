...
import { FlashGridSkeleton } from "../atom/FlashGridSkeleton";
...
export default function UserProfile({ user }: { user?: User }) {
  ...
  if (!neynarUserLoading && !neynarUser) {
    return (
...
`}
        ...
        <div className="bg-gray-900 border border-green-400 p-3 sm:p-4 mb-4">
          {neynarUserLoading ? (
            <div className="flex flex-col sm:flex-row items-center gap-3 sm:gap-4 animate-pulse">
              ...
              <div className="flex-1 flex flex-col items-center sm:items-start gap-3">
                ...
                <div className="h-8 w-40 bg-gray-800" />
              </div>
            </div>
          ) : (
      ...
      {/* User Feed */}
      {flashesLoading && <FlashGridSkeleton />}
      {!flashesLoading && flashes && Array.isArray(flashes) && flashes.length > 0 && (
