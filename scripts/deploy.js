...
async function lookupFidByCustodyAddress(custodyAddress, apiKey) {
  ...
  const response = await fetch(`https://api.neynar.com/v2/farcaster/user/bulk-by-address?addresses=${lowerCasedCustodyAddress}&address_types=custody_address`, {
    headers: {
...
async function generateFarcasterMetadata(domain, fid, accountAddress, seedPhrase, webhookUrl) {
  ...
  return {
    ...
    frame: {
      ...
      imageUrl: `https://${trimmedDomain}/frame-embed.png`,
  ...
  try {
    if (fs.existsSync(".env.local")) {
      ...
      if (loadLocal) {
        ...
        const allowedVars = ["SEED_PHRASE", "NEXT_PUBLIC_FRAME_NAME", "NEXT_PUBLIC_FRAME_DESCRIPTION", "NEXT_PUBLIC_FRAME_BUTTON_TEXT", "NEYNAR_API_KEY", "NEYNAR_CLIENT_ID"];
...
async function deployToVercel(useGitHub = false) {
  try {
    ...
    if (process.env.SEED_PHRASE) {
      ...
      const webhookUrl = process.env.NEYNAR_API_KEY && process.env.NEYNAR_CLIENT_ID ? `https://api.neynar.com/f/app/${process.env.NEYNAR_CLIENT_ID}/event` : `https://${domain}/api/webhook`;
      ...
      if (currentProject) {
        ...
        if (domainMatch) {
          ...
          if (actualDomain !== domain) {
            ...
            const webhookUrl =
              process.env.NEYNAR_API_KEY && process.env.NEYNAR_CLIENT_ID ? `https://api.neynar.com/f/app/${process.env.NEYNAR_CLIENT_ID}/event` : `https://${actualDomain}/api/webhook`;
